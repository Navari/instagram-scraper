...
class Endpoints
{
    ...
    const MY_PROFILE = 'https://www.instagram.com/graphql/query/?query_hash=c699b185975935ae2a457f24075de8c7&variables={"has_threaded_comments":true}';
    const CHECKPOINT_URL = 'https://i.instagram.com/api/v1/direct_v2/get_badge_count/?no_raven=1';
    ...
    const LIKE_URL = 'https://www.instagram.com/web/likes/{mediaId}/like/?hl=tr';
    ...
    const STORIES_URL_NEW = 'https://i.instagram.com/api/v1/feed/reels_tray/';
    const ACTIVE_FOLLOW_REQUESTS = 'https://www.instagram.com/accounts/access_tool/current_follow_requests?__a=1';
    const ACCOUNT_BLOCKED = 'https://www.instagram.com/accounts/access_tool/accounts_you_blocked?__a=1';
    const OLD_SEARCH_HISTORY = 'https://www.instagram.com/accounts/access_tool/search_history?__a=1';
    const STORY_VIEWER_LINK_START = 'https://i.instagram.com/api/v1/media/{mediaId}/list_reel_media_viewer/?include_blacklist_sample=true';
    const STORY_VIEWER_LINK = 'https://i.instagram.com/api/v1/media/{mediaId}/list_reel_media_viewer/?include_blacklist_sample=true&max_id={count}';
    const CONTACTS_LINK = 'https://www.instagram.com/graphql/query/?query_hash=68b15837c4c60cf5bb0c3df17a4791f8&variables={}';
    const LOCATION_SEARCH = 'https://www.instagram.com/web/search/topsearch/?context=place&query={query}';
    const SUGGESTED_USERS = 'https://www.instagram.com/graphql/query/?query_hash=ed2e3ff5ae8b96717476b62ef06ed8cc&variables={"fetch_media_count":0,"fetch_suggested_count":10,"ignore_cache":true,"filter_followed_friends":true,"seen_ids":[],"include_reel":false}';
    const FETCH_MUTUAL = 'https://www.instagram.com/graphql/query/?query_hash=5aefa9893005572d237da5068082d8d5&variables={"id":"{user_id}","include_reel":false,"fetch_mutual":true,"first":24}';
    const FETCH_CHAIN = 'https://www.instagram.com/graphql/query/?query_hash=d4d88dc1500312af6f937f7b804c68c3&variables={"user_id":"{user_id}","include_chaining":true,"include_reel":true,"include_suggested_users":false,"include_logged_out_extras":false,"include_highlight_reels":false,"include_live_status":false}';
    const SEARCH_HISTORY = 'https://www.instagram.com/accounts/access_tool/search_history?__a=1';
    const STORY_RESEARCHER = "https://i.instagram.com/api/v1/feed/reels_media/?reel_ids={{userid}}";
    ...
    public static function getStoryResearcher($user_id)
    {
        return str_replace('{{userid}}', urlencode($user_id), static::STORY_RESEARCHER);
    }
    ...
    {
        $url = self::getGraphQlUrl(Query::USER_STORIES, ['variables' => json_encode($variables)]);
        return $url;
    }
    ...
    {
        $url = self::getGraphQlUrl(Query::STORIES, ['variables' => json_encode($variables)]);
        return $url;
    }

    public static function getStoriesLinkNew(): string
    {
        return self::STORIES_URL_NEW;
    }
    ...
    public static function getLocationsUrl($query)
    {
        return str_replace('{query}', urlencode($query), static::LOCATION_SEARCH);
    }

    public static function getChainLink($query)
    {
        return str_replace('{user_id}', urlencode($query), static::FETCH_CHAIN);
    }
    public static function getMutualTest($query)
    {
        return str_replace('{user_id}', urlencode($query), static::FETCH_MUTUAL);
    }

    public static function getMutualLink($query)
    {
        return str_replace('{user_id}', urlencode($query), static::FETCH_MUTUAL);
    }
    ...
    public static function getStoryViewers($mediaId, $count = null)
    {
        if(is_null($count)){
            $url = static::STORY_VIEWER_LINK_START;
        }else{
            $url = static::STORY_VIEWER_LINK;
            $url = str_replace('{count}', $count, $url);
        }
        $url = str_replace('{mediaId}', $mediaId, $url);
        return $url;
    }

    public static function getOldFollowRequestUrl($cursor = null)
    {
        $url = static::ACTIVE_FOLLOW_REQUESTS;
